...
namespace Gameplay.Machines.Power
{
    public class EnergyGenerator : UpgradableEntity, IProductionTickable, IProducer
    {
        ...
        public void OnProductionTick()
        {
            ...
            if (generatedAmount <= 0f) return;

            if (_energyOutput.IsEmpty) _energyOutput = new InventorySlot(ItemCatalog.ENERGY, 0f);

            if (_energyOutput.Held != ItemCatalog.ENERGY) return;
        ...
        {
            if (request.IsEmpty || _energyOutput.IsEmpty || _energyOutput.Held != request.Held) return InventorySlot.Empty;
